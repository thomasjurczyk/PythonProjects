...
assignmentAverages = []
for currentAssignment in range(0,len(gradebook[0])):
    currentTotal = 0
    count = 0
    for currentStudent in gradebook:
        currentTotal += currentStudent[currentAssignment]
        count += 1
    assignmentAverages.append(currentTotal/count)
print("Assignment Averages:")
count = 0
for assignment in assignmentAverages:
    count += 1
    print("Assignment " + str(count) + ": " + '{0:.2f}'.format(assignment))
outerCount = 0
print("\nStudent Averages:")
for student in gradebook:
    ...
    for grade in student:
        currentTotal += grade
        count += 1
    print("Student " + str(outerCount) + ": " + '{0:.2f}'.format(currentTotal/count))
